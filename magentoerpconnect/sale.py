...
class SaleOrderImport(MagentoImportSynchronizer):
    ...
    def _get_magento_data(self):
        ...
        # we fix the record!
        # 'store_id' here is storeview_id
        if not record.get('website_id'):
            # deduce it from the store
            store_view_binder = self.get_binder_for_model('magento.storeview')
            oe_store_id = store_view_binder.to_openerp(record['store_id'])
            store = self.session.browse('magento.storeview', oe_store_id)
            # "fix" the record
            record['website_id'] = store.store_id.website_id.magento_id
        return record
